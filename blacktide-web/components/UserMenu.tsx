...
export default function UserMenu() {
  ...
  return (
    <Link href="/member" aria-label="會員中心" className="relative block h-8 w-8 shrink-0 overflow-hidden rounded-full"
      style={isFounder ? { boxShadow: `0 0 0 1.5px ${C.gold}, 0 0 8px ${C.gold}99` } : { boxShadow: "0 0 0 1px rgba(94,234,212,0.4)" }}>
      {avatar ? (
      ...
      {isFounder && (
        <span aria-label="創始會員" className="absolute -bottom-0.5 -right-0.5 flex h-3.5 w-3.5 items-center justify-center rounded-full text-[8px]" style={{ background: C.gold }}>
          🔥
        </span>
      )}
    </Link>
