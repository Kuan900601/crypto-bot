    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-white/10 bg-ink-800 p-5">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 text-xs">
          <div className="rounded-lg bg-white/[0.03] p-2.5"><div className="text-slate-500">進場區間</div><div className="mt-0.5 font-mono">{fmtPrice(s.entryLow)}–{fmtPrice(s.entryHigh)}</div></div>
          <div className="rounded-lg bg-white/[0.03] p-2.5"><div className="text-slate-500">止損</div><div className="mt-0.5 font-mono text-down">{fmtPrice(s.stopLoss)}</div></div>
          <div className="rounded-lg bg-white/[0.03] p-2.5"><div className="text-slate-500">槓桿</div><div className="mt-0.5 font-mono">{s.leverage}x</div></div>
        ...
        <button onClick={copy} className="mt-5 flex w-full items-center justify-center gap-2 rounded-lg bg-tide-500/15 py-2.5 text-sm font-semibold text-tide-300 hover:bg-tide-500/25">
          {copied ? <Check size={15} /> : <Copy size={15} />}{copied ? "已複製" : "複製完整下單計畫"}
        </button>
      </div>
    </div>
  );
