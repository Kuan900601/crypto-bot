  ...
  return (
    <Card onClick={onOpen} className="cursor-pointer p-4 transition-colors hover:border-tide-500/30">
      ...
      <div className="mt-3 grid grid-cols-3 gap-2 text-xs">
        <div><div className="text-slate-500">進場區間</div><div className="mt-0.5 font-mono">{fmtPrice(s.entryLow)}–{fmtPrice(s.entryHigh)}</div></div>
        <div><div className="text-slate-500">止損</div><div className="mt-0.5 font-mono text-down">{fmtPrice(s.stopLoss)}</div></div>
        <div><div className="text-slate-500">TP1（{s.rr}R）</div><div className="mt-0.5 font-mono text-up">{fmtPrice(s.tps[0]?.price ?? 0)}</div></div>
      </div>
      <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-slate-500">
        <span>評分 {s.score}</span>
        <div className="w-16"><Progress value={s.score} /></div>
        <span>勝率（推算）{s.winRate}%</span>
        <span className="ml-auto">{entryGradeDisplay(s.entryGrade)} · {s.leverage}x</span>
      </div>
