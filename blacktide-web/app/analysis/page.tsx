...
function DetailModal({ a, demo, onClose }: { a: RA; demo: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center p-0 sm:items-center sm:p-4">
      ...
      <div className="pop-in relative max-h-[88vh] w-full max-w-lg overflow-y-auto rounded-t-2xl border border-white/10 bg-ink-800 p-5 sm:rounded-2xl">
        <div className="flex items-center gap-2">
          <span className="text-lg font-bold">{a.symbol}</span>
          <Badge tone={biasTone(a.bias)}>{biasLabel(a.bias)}</Badge>
          {!demo && <span className="font-mono text-sm text-slate-300">${fmtPrice(a.price)}</span>}
          {!demo && <span className={"font-mono text-xs " + (a.change24h >= 0 ? "text-up" : "text-down")}>{a.change24h >= 0 ? "+" : ""}{a.change24h}%</span>}
        ...
        <div className="mt-3 text-[10px] leading-relaxed text-slate-600">{demo ? "目前為展示資料（即時指標暫時無法取得）。" : "以上為技術指標即時計算結果，僅供參考，不構成投資建議。"}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-5">
      <SectionTitle title="AI 智能分析"
        desc={demo ? "技術指標分析 · 點任一卡片看完整分析 · 非投資建議" : "技術指標即時計算（Bybit）· 點任一卡片看完整分析 · 非投資建議"} />
      <Card className="p-4">
      ...
      {demo && !loading && (
        <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-4 py-2.5 text-xs text-amber-200">即時指標暫時無法取得，以下為展示資料（仍可點開查看範例分析）。</div>
      )}
      {loading
        ? <div className="grid gap-4 lg:grid-cols-2">{[0, 1, 2, 3].map((i) => <Card key={i} className="h-52 animate-pulse" />)}</div>
        : (
            ...
            {display.map((a) => (
              <Card key={a.symbol} onClick={() => setSel(a)} className="cursor-pointer p-4 transition hover:border-tide-500/30">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Badge tone={biasTone(a.bias)}>{biasLabel(a.bias)}</Badge>
                  {!demo && <span className="font-mono text-xs text-slate-400">${fmtPrice(a.price)}</span>}
                  {!demo && <span className={`font-mono text-xs ${a.change24h >= 0 ? "text-up" : "text-down"}`}>{a.change24h >= 0 ? "+" : ""}{a.change24h}%</span>}
