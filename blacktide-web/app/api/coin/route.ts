...
function sma(a: number[], n: number) { if (a.length < n) return a[a.length - 1] ?? 0; const s = a.slice(-n).reduce((x, y) => x + y, 0); return s / n; }
...
function rnd(v: number) { return v >= 100 ? Math.round(v) : v >= 1 ? +v.toFixed(2) : +v.toPrecision(4); }
function clamp(v: number, lo = 0, hi = 100) { return Math.max(lo, Math.min(hi, Math.round(v))); }
async function analyzeSym(sym: string) {
  try {
    ...
    const basis = [
      ...
      "RSI(14) = " + Math.round(r) + "（" + (r > 70 ? "超買，留意回調" : r < 30 ? "超賣，留意反彈" : "中性") + "）",
...
export async function GET(req: Request) {
  const sym = (new URL(req.url).searchParams.get("symbol") || "").toUpperCase().replace(/[^A-Z0-9]/g, "");
  if (!sym) return Response.json({ ok: false, error: "缺少 symbol" }, { status: 400 });
  const a = await analyzeSym(sym);
  return Response.json(a ? { ok: true, analysis: a } : { ok: false });
}
