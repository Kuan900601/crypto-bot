...
interface AdminUser { email: string; name: string; phone: string; uid: string; tier: string; cycle?: string; subAmount?: number; planExpiry?: string; createdAt: string; }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
        ...
        <Stat label="免費 / Air / Pro" value={(s?.free ?? 0) + " / " + (s?.air ?? 0) + " / " + (s?.pro ?? 0)} />
        ...
        <Stat label="Air 訂閱數" value={String(s?.air ?? 0)} />
        ...
        <Card className="overflow-x-auto p-0">
          <table className="w-full min-w-[680px] text-left text-xs">
            <thead className="border-b border-white/5 text-slate-500"><tr>
              <th className="px-3 py-2">姓名 / UID</th><th className="px-3 py-2">聯絡</th><th className="px-3 py-2">方案</th><th className="px-3 py-2">到期</th><th className="px-3 py-2">註冊</th><th className="px-3 py-2">設定等級</th>
            </tr></thead>
            <tbody>
              {(data?.users || []).map((u) => (
                <tr key={u.email} className="border-b border-white/5 last:border-0">
                  ...
                  <td className="px-3 py-2">
                    <select defaultValue={u.tier + ":" + (u.cycle || "monthly")} onChange={(e) => { const [t, c] = e.target.value.split(":"); setTier(u.email, t, c); }}
                      ...
                      <option value="free:monthly">免費</option><option value="air:monthly">Air · 月</option><option value="air:yearly">Air · 年</option><option value="pro:monthly">Pro · 月</option><option value="pro:yearly">Pro · 年</option>
                    </select>
                  </td>
                </tr>
              ))}
            ...
            <tbody>
              {(data?.payments || []).map((p) => (
                <tr key={p.id} className="border-b border-white/5 last:border-0">
                  ...
                  <td className="px-3 py-2">{p.tier?.toUpperCase()} · {p.cycle === "yearly" ? "年" : "月"}</td>
