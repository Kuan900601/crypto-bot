...
- auto_trader.py：`at:` 系列 key（實測：`at:processed_signals`、`at:processed_closes`、`at:trades`、`at:pending`、`at:day_equity`、`at:breaker_tripped`、`at:liquidations`、`at:pnl_ledger`、`at:heartbeat`、`at:last_cycle`、`at:events`）。bot.py 另寫 `bt:last_scan`（主掃描耗時）。狀態存 Redis 以根治「換容器重放 backlog」。

## 3. 自動交易（執行層）詳解【真錢，謹慎】

- 模式：真錢（`USE_SANDBOX=False`，作者決定）。總開關 `AUTO_TRADE_ENABLED`（預設關）。允許等級 `AUTO_TRADE_TIERS`（預設 `S,A,B`）。
- 風險（已知，由作者承擔）：`AT_LEVERAGE`＝20、最多 `AT_MAX_POSITIONS`＝4 倉；極端情況多倉同時觸損可達淨值大幅回撤；日內熔斷 `MAX_DAILY_DD`（預設 0.10）跌破當日起始即停開新倉。帳戶只放可承受全損的金額。倉位 `SIZING_MODE`（預設 `fixed`＝淨值÷4；`risk` 模式用 `RISK_PER_TRADE_PCT`，預設 0.03）。
...
- 移動止損階梯（v61，給回踩留緩衝、不再死保本）：TP1 成交後 SL 移到 `entry−緩衝`（緩衝 = `max(TRAIL_BUFFER_MIN_PCT, 0.5×ATR/price)`，`TRAIL_BUFFER_MIN_PCT` 預設 0.004）；TP2 後移到 `entry`（此時才真保本）；TP3 後移到 TP1。每段只做一次（`sl_stage`），用 V5 trading-stop 原子更新位階止損。
...
- **快速動能搶先偵測（v61）**：`fast_momentum_scan`（bot.py 背景 job，interval 90s）對 24h 漲跌幅前 12 + 量能異常的幣，用 5m/15m 跑 `fast_breakout_check`（放量突破近期高/低 + 5m 連續同向 + 量>1.8x），命中即走正常 `register_signal`+推播+佇列流程，標記「⚡快速動能」、tier=B、order_type=MARKET，受連虧/熔斷管制。強度門檻 `FAST_MOMENTUM_MIN_STRENGTH`（預設 55）。用途是降低「噴一段才推」的延遲。
...
當前 `BOT_VERSION = "v61"`（版本字串一度停在 v56，碼早已超前，v61 補正）。
...
自動交易：`AUTO_TRADE_ENABLED`（預設 false）、`AUTO_TRADE_TIERS`（預設 `S,A,B`）、`AT_LEVERAGE`（預設 20）、`AT_MAX_POSITIONS`（預設 4）、`AT_MAX_SL_PCT`（預設 0.035）、`SIZING_MODE`（預設 `fixed`）、`RISK_PER_TRADE_PCT`（預設 0.03）、`MAX_DAILY_DD`（預設 0.10）、`LIMIT_ORDERS`（預設 true）、`SIGNAL_MAX_AGE_MIN`（預設 10）、`MAX_ENTRY_DRIFT_PCT`（預設 1.5）、`TRAIL_BUFFER_MIN_PCT`（預設 0.004）。
推播/策略：`C_TIER_DELAY_MIN`（預設 30）、`FAST_MOMENTUM_MIN_STRENGTH`（預設 55）、`STRICT_CONTEXT_GATE`（情境閘門，預設開）、`EMERGENCY_SIGNALS`（預設 false）。
