...
# v65 P4：止損 ATR 乘數縮放（預設 1.0 = 不改現有行為；>1 放寬止損，<1 收緊）
_SL_ATR_MULT_SCALE = float(os.getenv("SL_ATR_MULT", "1.0"))

# v66 P1：情境閘門收緊（閾值收 0.90→0.80、0.10→0.20，加軟懲罰與趨勢末端懲罰）
_RP_LONG_MAX = float(os.getenv("RP_LONG_MAX", "0.80"))       # 硬擋：高位多入閾值（原 0.90）
_RP_SHORT_MIN = float(os.getenv("RP_SHORT_MIN", "0.20"))      # 硬擋：低位空入閾值（原 0.10）
_CHASE_RP_PENALTY = int(os.getenv("CHASE_RP_PENALTY", "10"))  # 軟懲罰：接近高/低位追入
_CHASE_REGIME_PENALTY = int(os.getenv("CHASE_REGIME_PENALTY", "8"))  # 軟懲罰：強趨勢末端入場
...
class CryptoAnalyzer:
    ...
    def __init__(self):
        ...
        self._gate_shadow_queue = []  # v66 P1：硬擋記錄佇列，bot.py 每輪掃描後 flush 到 Redis gate_shadow
    ...
    def smart_stop_loss(self, direction, entry, df, atr_v, sw_res, sw_sup,
        ...
        try:
            ...
            pass
        # v65 P4：SL_ATR_MULT env 縮放（預設 1.0 無影響；作者可用此 env 統一放寬/收緊 ATR 止損）
        if _SL_ATR_MULT_SCALE != 1.0:
            atr_mult *= _SL_ATR_MULT_SCALE
        candidates = []
        labels = []
        if direction == "LONG":
            # 1. ATR 止損
    ...
    def entry_context_gate(self, direction, sig1h, sig4h, df1h, df4h,
        ...
        info = {"range_pos": None, "tags": [], "chase_flags": [], "score_penalty": 0, "regime": ""}
        try:
            p = float(current_price)
            look = df1h.tail(_RANGE_POS_LOOKBACK)  # v66 P1：env 化（預設 120）
            ...
            info["regime"] = r4  # 供 professional_setup _gate_shadow_queue 使用
            adx = sig1h.get("adx", 0); rsi = sig1h.get("rsi", 50)

            # 1) 區間位階：高位追多/低位追空，除非 4H BOS 同向或強勢續航（v66 P1 閾值收緊）
            if direction == "LONG" and range_pos > _RP_LONG_MAX and bos4 != "BULL_BOS" and not cont_ok:
                return False, "高位追多 pos=%.2f 無4H突破/續航" % range_pos, info
            if direction == "SHORT" and range_pos < _RP_SHORT_MIN and bos4 != "BEAR_BOS" and not cont_ok:
            ...
            # v66 P1：軟懲罰區間 + chase_flags（通過硬擋但仍在危險區）
            _chase_flags = []
            _penalty = 0

            # 1a) 接近高/低位追入（軟懲罰）
            _rp_soft_long = 0.65 <= range_pos <= _RP_LONG_MAX
            _rp_soft_short = _RP_SHORT_MIN <= range_pos <= 0.35
            if direction == "LONG" and _rp_soft_long:
                if bos4 == "BULL_BOS" or cont_ok:
                    _chase_flags.append("CHASE_RP_EXCEPTION")   # 有例外條件，允許但標記
                else:
                    _chase_flags.append("CHASE_RP")
                    _penalty += _CHASE_RP_PENALTY
            if direction == "SHORT" and _rp_soft_short:
                if bos4 == "BEAR_BOS" or cont_ok:
                    _chase_flags.append("CHASE_RP_EXCEPTION")
                else:
                    _chase_flags.append("CHASE_RP")
                    _penalty += _CHASE_RP_PENALTY

            # 1b) 強趨勢末端入場懲罰（STRONG_BULL 追多 / STRONG_BEAR 追空 = 共識最高 = 末端）
            if _CHASE_REGIME_ENABLED:
                if direction == "LONG" and r4 == "STRONG_BULL":
                    _chase_flags.append("CHASE_REGIME")
                    _penalty += _CHASE_REGIME_PENALTY
                if direction == "SHORT" and r4 == "STRONG_BEAR":
                    _chase_flags.append("CHASE_REGIME")
                    _penalty += _CHASE_REGIME_PENALTY

            info["chase_flags"] = _chase_flags
            info["score_penalty"] = _penalty
    ...
    def professional_setup(self, sig1h, sig4h, sig15m, df1h, df4h, df_daily,
        ...
        # ⭐ v57 情境閘門：高位不追多/低位不追空、4H強反不逆勢、盤整只做邊緣反轉
        _gate_info = {"range_pos": None, "tags": [], "chase_flags": [], "score_penalty": 0, "regime": ""}
        if os.getenv("STRICT_CONTEXT_GATE", "true").lower() == "true":
            ...
            if not _ok_gate:
                ...
                try:
                    ...
                    self._gate_shadow_queue = self._gate_shadow_queue[-300:]
                except Exception:
                    pass
                return None, "情境閘門: " + _gate_reason
            # v66 P1：通過硬擋後，套用軟懲罰（chase_flags 已在 _gate_info 中）
            _sp = _gate_info.get("score_penalty", 0)
            if _sp > 0:
                score -= _sp
